...
public class BasicNode
{
	...
	public static final void main(String[] args)
	{
		...
		{
			System.out.println("Seed HashName :: " + seed.getHashName());
			for (Map.Entry<CipherSetIdentifier, byte[]> entry : seed.getFingerprints().entrySet())
			{
				System.out.println("cs" + entry.getKey() + " fingerprint: " + Util.bytesToHex(entry.getValue()));
			}
			for (CipherSetIdentifier csid : seed.getCipherSetIds())
			{
				...
				try
				{
					...
					if (publicKey != null)
					{
						System.out.println("pub: " + Util.base64Encode(seed.getPublicKey(csid).getEncoded()));
						System.out.println("fpr: " + Util.bytesToHex(seed.getPublicKey(csid).getFingerprint()));
					}
				} catch (TelehashException e)
				{
					// TODO Auto-generated catch block
